...
adj <- matrix(c(0,0,0,0.5,0,0,0.5,2,0), 3, 3)
rownames(adj) <- colnames(adj) <- 1:3
g1 <- as(adj, "graphNEL")
d1 <- simulate_data(g1, dist.dispersion = 10)
g2 <- resample_edge_weights(g1, lB = c(0.5,2), uB = c(0.5,2))
d2 <- simulate_data(g2, dist.dispersion = 10)
adj - as(g2, "matrix")
compute_differential_causal_effects(g1, d1, g2, d2)
