...
A <- rnbinom(1000, size=100, mu=10)
B <- rnbinom(1000, size=100, mu=exp(log(10) + 1.6 * (A - mean(A))))

lib.factor <- sample(c(1, 2, 4, 8), 1000, replace=TRUE)
...
glm.fun(B.s ~ A.s * lib.factor) # works fine (but `lib.factor` is ground truth)
...
pdf("temp2.pdf", width = 50, height = 50)
