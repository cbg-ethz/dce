...
set.seed(9247)
dag <- create_random_DAG(n, runif(1, 0, 1), lB, uB)
n <- 100
dag <- matrix(0, n, n)
dag[upper.tri(dag)] <- 1
dag <- transitive.reduction(dag)
rownames(dag) <- colnames(dag) <- 1:n
dag <- as(dag, "graphNEL")
dag <- resample_edge_weights(dag, lB, uB)
dag2 <- resample_edge_weights(dag, lB, uB)
wt <- simulate_data(dag, m[1], link.log.base = 0)
mt <- simulate_data(dag2, m[2], link.log.base = 0)

E <- compute_differential_causal_effects(dag, wt, dag2, mt, link.log.base = 0)
cor(as.vector(E$dce), as.vector(trueEffects(dag) - trueEffects(dag2)))
plot(as.vector(E$dce), as.vector(trueEffects(dag) - trueEffects(dag2)))#, xlim = c(-2,2))

```

```{r}
getMinAdjSets <- function(g, x = NULL, y = NULL, type = "dce") {
    a <- as(g, "matrix")
    a[which(a != 0)] <- 1
    n <- nrow(a)
    atc <- mnem:::mytc(a)
    ...
    for (i in intersect(seq_len(n-1),x)) {
        ...
        for (j in intrsect((i+1):n, y)) {
            if (sum(a[, j] == 1) == 1 | a[i, j] == 0) {
                Z[[i]][[j]] <- NULL
                next()
            } else {
                if (type %in% "pcalg") {
                    Z[[i]][[j]] <- pcalg::backdoor(a, i, j, type = "dag")
                } else {
                    ptc <- which(atc[, i] == 1)
                    ptc <- ptc[-which(ptc == i)]
                    Z0 <- NULL
...
beta <- 0.6
set.seed(42)
A <- rnbinom(1000, size=0.1, mu=1000)
hist(A)
betaA <- beta * scale(A)
B <- rnbinom(1000, size=Inf, mu= exp(log(1000 - max(betaA)) + betaA)) # link function keeps mu positive, exp can lead to extreme values
hist(B)
...
# helper function
log1 <- list(linkfun = function(mu) log(mu, 1.001),
        linkinv = function(eta) 1.001^eta,
        mu.eta = function(eta) 1.001^eta,
        valideta = function(eta) TRUE,
        name = "log1")
class(log1) <- "link-glm"

glm.fun <- function(...) {
  glm2::glm2(..., family = MASS::negative.binomial(theta=10, link="log"), control=list(maxit=1000))
}
