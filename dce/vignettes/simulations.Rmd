...
lB <- c(0.5,2) # negative effects intervall
uB <- c(0.5,2) # positive effects intervall
...
adj <- matrix(c(0,0,0,0,
                ...
                1.5,0,0,0,
                1.25,1.2,0.7,0), 4, 4)
rownames(adj) <- colnames(adj) <- 1:4
g1 <- as(adj, "graphNEL")
g2 <- resample_edge_weights(g1, c(0.5,2), c(0.5, 2))
d1 <- simulate_data(g1, 100, 100, 100)
d2 <- simulate_data(g2, 100, 100, 100)
Pce <- adj - as(g2, "matrix")
compute_differential_causal_effects(g1, d1, g2, d2, theta = 100, partial = 1)$dce
Tce <- trueEffects(g1, partial = 0) - trueEffects(g2, partial = 0)
tmp <- compute_differential_causal_effects(g1, d1, g2, d2, theta = 100, partial = 0)$dce
tmp
computeEdgeWeights <- function(g, dce) {
    mat <- as(g, "matrix")
    mat[which(mat != 0)] <- 1
    pdce <- mat*dce
    for (i in 1:nrow(mat)) {
