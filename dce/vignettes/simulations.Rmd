...
```{r}
## library(dce)
source("~/CausalPathways/dce/R/main.R"); source("~/CausalPathways/dce/R/utils.R"); source("~/CausalPathways/dce/R/simulations.R"); library(tidyverse); library(purrr); library(graph); library(pcalg); library(assertthat); library(igraph); library(matlib); library(nem); library(mnem); library(expm); library(Linnorm); library(MASS)
```

```{r}
## local test parameters:
## genes <- 10; tumor <- 200; normal <- 200; runs <- 100; perturb <- 0; prob <- "runif"; bootstrap <- c(0, "full", "basic"); truepos <- 1; test <- 0
...
source("/Users/mpirkl/Documents/CausalPathways/dce/R/main.R"); source("/Users/mpirkl/Documents/CausalPathways/dce/R/utils.R"); source("/Users/mpirkl/Documents/CausalPathways/dce/R/simulations.R"); 
simRes <- simDce(n,m,runs,mu=mu,sd,c(lB,uB),truepos,perturb,method,p,bootstrap=bootstrap,verbose=TRUE,test=test,errDist=errDist)

col <- rgb(c(0.5,1,0,0.1),c(0.5,0,0,0.1),c(0.5,0,1,0.1),0.75)
plot.dceSim(simRes, col = col, border = col, showMeth = c(2,5,3,6,1,4), showFeat = c(2,1,4,5,6,3), methNames = c("basic", "basic\n bootstrap", "full", "full\n bootstrap", "random", "correlation"))
...
Copy the updated files to the cluster environment. Set the folders accordingly (e.g. CausalPathways as the working directory).
