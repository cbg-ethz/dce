...
set.seed(42)
size <- 10; mu <- 10
A <- rnbinom(1000, size=size, mu=mu)
B <- rnbinom(1000, size=size, mu=exp(log(10) + beta * (A  - mean(A)))) # link function keeps mu positive, exp can lead to extreme values

lib.fac <- sample(1:2, 1000, replace = TRUE)

A.s <- A*
B.s <- B*lib.fac

lib.size <- apply(rbind(A.s, B.s), 2, sum)
...
lib.est <- y$samples$norm.factors/lib.size

linkfun <- "log"
glm2::glm2(B ~ A, family=MASS::negative.binomial(theta=10, link=linkfun))

glm2::glm2(B.s ~ A.s, family=MASS::negative.binomial(theta=10, link=linkfun))

glm2::glm2(B.s ~ A.s*lib.fac, family=MASS::negative.binomial(theta=10, link=linkfun))

glm2::glm2(data[, 2] ~ data[, 1] + data[, 1]*lib.est, family=MASS::negative.binomial(theta=10, link=linkfun), control = glm.control(epsilon = 1e-08, maxit = 100, trace = FALSE))
...
rownames(brca.kegg) <- colnames(brca.kegg) <- unlist(lapply(genelist$hsa05224, function(x) {
...
genes <- biomaRt::getBM(attributes=c('ensembl_gene_id','hgnc_symbol'),
      ...
      mart = ensembl)
brca.adj <- brca.kegg
rownames(brca.adj) <- colnames(brca.adj) <- genes$ensembl_gene_id
...
for (i in 1:length(stage)) {
    ...
    rownames(brca.adj) <- colnames(brca.adj) <- genes$hgnc_symbol
    ...
    edgecol <- rgb(brca.col, 0, max(brca.col) - brca.col)
    mnem::plotDnf(brca.dnf, edgecol = edgecol, edgewidth = log(round(abs(brca.freq), 2)*100+0.1), edgelabel = round(brca.freq, 2)*100,
                  main = names(stage)[i])
}
