...
```{r}
res <- dce(graph_mt, X_wt, X_mt, deconfounding=F)
qplot(truth[truth!=0], res$dce[truth!=0]) + geom_abline(color='red', linetype='dashed') + xlab('true DCE') + ylab('estimated DCE')
...
res_without_deconf <- dce(graph_mt, bX_wt, bX_mt, deconfounding=F)

cowplot::plot_grid(
  plot_network(graph_wt, value_matrix=-log(res_without_deconf$dce_pvalue + epsilon)) + ggtitle("-log(p-values) without deconfounding"),
  qplot(truth[truth!=0], res_without_deconf$dce[truth!=0]) + geom_abline(color='red', linetype='dashed') + xlab('true DCE') + ylab('estimated DCE'), nrow=1
)
```
...
cowplot::plot_grid(
  plot_network(graph_wt, value_matrix=-log(res_with_deconf$dce_pvalue + epsilon)) + ggtitle("-log(p-values) with deconfounding"),
  qplot(truth[truth!=0], res_with_deconf$dce[truth!=0]) + geom_abline(color='red', linetype='dashed') + xlab('true DCE') + ylab('estimated DCE'), nrow=1
)
```
